    ...
    const load = React.useCallback(async () => {
        ...
        try {
            ...
            const { data, error: err } = await supabase
                ...
                .select('id,snapshot_month,organisation_id,organisation_name,total_open_cases,overdue_open_cases,sla_compliance_percent,safeguarding_score,generated_at')
            ...
            <div className="dashboard-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    <button
                        ...
                        style={{ height: 44, padding: '0 14px', borderRadius: 12, display: 'inline-flex', alignItems: 'center', gap: 8, opacity: generating ? 0.7 : 1 }}
                    >
                                ...
                                <tbody>
                                    {rows.map(r => (
                                        <tr key={r.id}>
                                            ...
                                            <td>{r.organisation_name ?? r.organisation_id ?? '—'}</td>
